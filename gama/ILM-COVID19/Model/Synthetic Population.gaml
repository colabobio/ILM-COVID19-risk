...
global {
	...
	action create_population_from_file(map<Building,float> working_places,map<list<int>,list<Building>> schools, list<Building> homes
		...
		with:[
			...
			has_preexisting_condition::convert_has_preexisting_condition(get(has_preexisting_condition_var)),
	...
	// Convert SP encoded has_preexisting_condition into gama model specification
	bool convert_has_preexisting_condition(string input){
		input <- input replace("\"","");
