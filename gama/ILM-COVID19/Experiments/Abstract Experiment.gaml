...
experiment "Abstract Experiment" virtual:true{
	...
	output {
		display "default_display" synchronized: false type: opengl background: background virtual: true draw_env: false {
			...
			//adding outside agents too..
			agents "Individual"  value: Individual where (each.is_outside){
				draw circle(status=susceptible or status=recovered? 10: 20) color: status = latent ? #black : (self.is_infectious ? #red : (status = recovered?#blue:#green)) ;	
			}
		}
